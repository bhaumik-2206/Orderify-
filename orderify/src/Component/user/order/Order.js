                    ...
                    <div key={index}>
                        ...
                        <div className='grid grid-cols-2'>
                            {orders[item].map((dateArray, index) => (
                                <div onClick={() => handleCheckBoxChange(dateArray.product_details._id, dateArray.prd_total_qty, item)} key={index}
                                    className={`${buyAgainOrder.find(ele => ele.cartitm_fk_prd_id === dateArray.product_details._id && ele.date === item) ? "bg-green-100 border border-green-700" : ""} m-2 py-4 cursor-pointer border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center`}>
